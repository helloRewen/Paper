...
RESULTS_DIR = ROOT / "results" / "main" / "prefix_10_90_baseline"
...
def choose_prefix_len(seq: Sequence[str], ratio: float) -> int:
    n = len(seq)
...
def build_prefix_table(df: pd.DataFrame, labels: pd.DataFrame, ratio: float) -> pd.DataFrame:
    ...
    for caseid, group in df.groupby("caseid", sort=False):
        ...
        seq = group["activity_alias"].tolist()
        times = group["dateTime"].tolist()
        prefix_len = choose_prefix_len(seq, ratio)
        seq_prefix = seq[:prefix_len]
        time_prefix = times[:prefix_len]
...
def fit_sequence_model(prefix_df: pd.DataFrame, ratio: float, model_name: str, cutoff: pd.Timestamp) -> tuple[dict, pd.DataFrame]:
...
def main() -> None:
    ...
    labels.to_csv(RESULTS_DIR / "full_labels.csv", index=False, encoding="utf-8-sig")
    ...
    for ratio in PREFIX_RATIOS:
        ...
        prefix_tables[ratio] = prefix_df
        prefix_df.to_csv(RESULTS_DIR / f"prefix_table_{int(ratio * 100)}.csv", index=False, encoding="utf-8-sig")
        ...
        metrics_df["prefix_ratio"] = ratio
        metrics_df["train_cases"] = len(train_df)
        metrics_df["test_cases"] = len(test_df)
        metrics_df["time_cutoff"] = str(cutoff)
        all_metric_rows.extend(metrics_df.to_dict(orient="records"))
        pred_df["prefix_ratio"] = ratio
        all_prediction_frames.append(pred_df)
        print(f"classical ratio={ratio:.1f} done")

    for model_name in ["LSTM", "Transformer"]:
        set_seed(SEED)
        for ratio in PREFIX_RATIOS:
            metrics, pred_df = fit_sequence_model(prefix_tables[ratio], ratio, model_name, cutoff)
            all_metric_rows.append(metrics)
            all_prediction_frames.append(pred_df)
            print(
                f"{model_name} ratio={ratio:.1f}: "
                f"F1={metrics['f1']:.4f}, AP={metrics['ap']:.4f}, Brier={metrics['brier']:.4f}"
            )

    metrics_out = pd.DataFrame(all_metric_rows)
    metrics_out = metrics_out[
        [
            ...
            "train_cases",
            "test_cases",
            *([c for c in ["val_cases", "vocab_size", "best_epoch", "best_val_ap", "time_cutoff"] if c in metrics_out.columns]),
        ]
    ]
    metrics_out["prefix_percent"] = (metrics_out["prefix_ratio"] * 100).round().astype(int).astype(str) + "%"
    metrics_out.to_csv(RESULTS_DIR / "prefix_10_90_baseline_metrics.csv", index=False, encoding="utf-8-sig")
    pd.concat(all_prediction_frames, ignore_index=True).to_csv(
        RESULTS_DIR / "prefix_10_90_baseline_predictions.csv", index=False, encoding="utf-8-sig"
    )
